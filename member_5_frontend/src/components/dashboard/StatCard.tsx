...
export const StatCard: React.FC<StatCardProps> = ({
  ...
  return (
    <div className={`p-5 bg-slate-900/90 rounded-xl border ${colorStyles.border} ${colorStyles.glow} relative overflow-hidden group hover:border-slate-700 transition-all`}>
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs font-mono font-medium text-slate-400 uppercase tracking-wider">{title}</p>
          <h3 className="text-3xl font-extrabold font-mono text-slate-100 mt-2 tracking-tight">{value}</h3>
      ...
      {trend && (
        <div className="mt-3 pt-3 border-t border-slate-800/80 flex items-center text-[11px] font-mono text-slate-400">
