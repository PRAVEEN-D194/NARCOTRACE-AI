...
export const Navbar: React.FC<NavbarProps> = ({ onToggleMobile }) => {
  ...
  return (
    <header className="h-14 bg-white/90 dark:bg-slate-900/90 border-b border-slate-200 dark:border-slate-800 backdrop-blur px-4 sm:px-6 flex items-center justify-between shrink-0 relative z-10 transition-colors duration-200">
      {/* Left items: Mobile Menu Button & Clearance Header */}
      <div className="flex items-center space-x-3">
        ...
        <div className="flex items-center space-x-2 sm:space-x-3">
          <div className="flex items-center space-x-1.5 px-2.5 py-1 rounded bg-amber-500/10 border border-amber-500/30 text-amber-700 dark:text-amber-400 text-xs font-mono font-semibold">
          ...
          {/* Synthetic / Demonstration Data Badge */}
          <div className="hidden md:flex items-center space-x-2 text-xs font-mono px-2.5 py-1 rounded bg-blue-500/10 border border-blue-500/30 text-blue-700 dark:text-blue-300">
            <Database className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
            <span>
              MODE: <strong>Synthetic / Demonstration Data</strong>
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-2 sm:space-x-3">
        ...
        <button className="relative p-2 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors border border-slate-200 dark:border-slate-800">
          ...
          <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-blue-500 dark:bg-blue-400 rounded-full" />
        </button>
      </div>
    </header>
  );
