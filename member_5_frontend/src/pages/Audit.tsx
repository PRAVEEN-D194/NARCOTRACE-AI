  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-200 dark:border-slate-800">
        ...
        <div className="flex items-center space-x-2 text-xs font-mono text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/60 px-3 py-1.5 rounded-lg border border-emerald-300 dark:border-emerald-500/30">
      ...
      {/* Search Bar */}
      <div className="p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl flex items-center justify-between gap-4 shadow-sm dark:shadow-xl transition-colors duration-200">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 text-xs font-mono">
                {filteredLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="py-3.5 px-4 text-slate-500 dark:text-slate-400 text-[11px]">{log.timestamp}</td>
                    <td className="py-3.5 px-4 font-bold text-slate-800 dark:text-slate-200">{log.investigatorName}</td>
                    <td className="py-3.5 px-4 font-bold text-cyan-600 dark:text-cyan-400">{log.action}</td>
                    <td className="py-3.5 px-4 text-slate-700 dark:text-slate-300">{log.caseId}</td>
                    <td className="py-3.5 px-4">
                      <Badge variant={log.result === 'Success' ? 'active' : 'high'}>
                        {log.result}
                      </Badge>
                    </td>
                    <td className="py-3.5 px-4 text-slate-500 dark:text-slate-400 text-[11px]">{log.ipAddress}</td>
                    <td className="py-3.5 px-4 text-slate-600 dark:text-slate-400 font-sans text-xs">{log.details || '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
