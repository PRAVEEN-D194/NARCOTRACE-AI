    ...
    <div className="space-y-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {cases.map((c) => (
            <div
              ...
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-cyan-500/40 rounded-xl p-5 shadow-sm dark:shadow-xl transition-all duration-200 hover:shadow-md dark:hover:shadow-2xl cursor-pointer flex flex-col justify-between space-y-4 group"
            >
              <div className="space-y-3">
                {/* ID & Priority Header */}
                <div className="flex items-center justify-between">
                  <span className="text-sm font-mono font-bold text-cyan-600 dark:text-cyan-400 group-hover:underline">
                    {c.id}
                  </span>
                  <div className="flex items-center space-x-2">
                ...
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {c.aliases.map((a, i) => (
                    <span key={i} className="text-[10px] font-mono px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-950 text-cyan-700 dark:text-cyan-300 border border-slate-200 dark:border-slate-800">
                      @{a.name} ({a.platform})
                    </span>
                  ))}
                </div>
              ...
              <div className="pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between text-xs font-mono text-slate-500 dark:text-slate-400">
                ...
                <span className="text-cyan-600 dark:text-cyan-400 group-hover:translate-x-1 transition-transform">
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-6 max-w-lg w-full shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
              <h3 className="font-mono font-bold text-base text-slate-900 dark:text-slate-100">Initialize New Case Dossier</h3>
            ...
            <form onSubmit={handleCreateCase} className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-mono text-slate-700 dark:text-slate-300">Target Subject / Handle</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full px-3 py-2 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 text-xs font-mono rounded border border-slate-200 dark:border-slate-800 focus:border-cyan-500 focus:outline-none"
                />
              </div>

              <div className="space-y-1">
                <label className="text-xs font-mono text-slate-700 dark:text-slate-300">Investigation Title</label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                <label className="text-xs font-mono text-slate-700 dark:text-slate-300">Initial Priority Level</label>
                <select
                  value={newPriority}
