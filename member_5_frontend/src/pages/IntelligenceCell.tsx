  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-800">
        ...
        <div className="flex items-center space-x-2 text-xs font-mono text-cyan-400 bg-cyan-950/60 px-3 py-1.5 rounded-lg border border-cyan-500/30">
      ...
      <div className="p-4 bg-slate-900 border border-slate-800 rounded-xl flex items-start space-x-3 text-xs">
        ...
        <div className="space-y-1">
          <h3 className="font-mono font-bold text-slate-100 uppercase">
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {findings.map((item) => (
            <div
              key={item.id}
              className={`p-6 rounded-xl border space-y-4 shadow-xl relative overflow-hidden ${
                item.isControlled
            ...
            >
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 pb-3 font-mono">
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-xs font-bold text-slate-300">
                    Confidence: <strong className="text-cyan-400">{item.confidence}%</strong>
                  </span>
                </div>

                <div className="flex items-center space-x-3 text-xs">
                  <span className="text-slate-400">Classification: <strong className="text-amber-400">{item.classification}</strong></span>
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-300 leading-relaxed font-sans bg-slate-950 p-3.5 rounded-lg border border-slate-800">
                  {item.summary}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs font-mono pt-2">
                <div className="p-2.5 bg-slate-950 rounded-lg border border-slate-800">
                  <span className="text-slate-500 text-[10px]">CORRELATED ENTITIES</span>
                  <p className="font-bold text-cyan-300 truncate">{item.relatedEntities.join(', ')}</p>
                </div>
                <div className="p-2.5 bg-slate-950 rounded-lg border border-slate-800">
                  <span className="text-slate-500 text-[10px]">ATTACHED EVIDENCE</span>
                  <p className="font-bold text-slate-200 truncate">{item.evidenceIds.join(', ') || 'None'}</p>
                </div>
                <div className="p-2.5 bg-slate-950 rounded-lg border border-slate-800">
                  <span className="text-slate-500 text-[10px]">VALIDATION STATUS</span>
                  <p className="font-bold text-emerald-400 truncate">{item.validationStatus}</p>
                </div>
                <div className="p-2.5 bg-slate-950 rounded-lg border border-slate-800">
                  <span className="text-slate-500 text-[10px]">BACKEND CLEARANCE</span>
                  <p className="font-bold text-amber-400 truncate">{item.authorizationStatus}</p>
                </div>
              </div>

              {/* Security Warning for RAW signal */}
              {!item.isControlled && (
                <div className="p-3 bg-rose-950/40 border border-rose-500/30 rounded-lg flex items-center space-x-2 text-xs font-mono text-rose-300">
